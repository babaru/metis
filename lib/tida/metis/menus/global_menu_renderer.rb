    ...
    module Menus
      class GlobalMenuRenderer < ::SimpleNavigation::Renderer::Base
        ...
        def render(item_container)
          ...
          content_tag(:ul, list_content, class: "nav navbar-nav")
        end
        ...
        def a_tag_for(item, options)
          ...
          link_to([
              icon_content,
              (options['data-toggle'] ? [item.name, top_level ? fa_icon('angle-down caret') : fa_icon('angle-right caret')].join(' ') : item.name),
            ].join(' '),
            item.url,
            options
            )
